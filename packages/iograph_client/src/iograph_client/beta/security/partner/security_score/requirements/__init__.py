...
class RequirementsRequest(BaseRequestBuilder):
	def __init__(self,request_adapter: HttpxRequestAdapter, path_parameters: Optional[Union[dict[str, Any], str]]) -> None:
		super().__init__(request_adapter, "{+baseurl}/security/partner/securityScore/requirements", path_parameters)

	async def get(
		self,
		request_configuration: Optional[RequestConfiguration[GetQueryParams]] = None,
	) -> PartnerSecuritySecurityRequirementCollectionResponse:
		"""
		List requirements
		...
		return await self.request_adapter.send_async(request_info, PartnerSecuritySecurityRequirementCollectionResponse, error_mapping)
	...
	def by_security_requirement_id(self,
		securityRequirement_id: str,
	) -> BySecurityRequirementIdRequest:
		if securityRequirement_id is None:
			raise TypeError("securityRequirement_id cannot be null.")

		path_parameters = get_path_parameters(self.path_parameters)
		path_parameters["securityRequirement%2Did"] =  securityRequirement_id

		from .by_security_requirement_id import BySecurityRequirementIdRequest
		return BySecurityRequirementIdRequest(self.request_adapter, path_parameters)
