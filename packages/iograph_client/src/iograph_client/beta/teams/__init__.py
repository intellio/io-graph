# Auto-generated client

from __future__ import annotations
from kiota_abstractions.get_path_parameters import get_path_parameters
from kiota_abstractions.method import Method
from kiota_abstractions.base_request_builder import BaseRequestBuilder
from kiota_abstractions.base_request_configuration import RequestConfiguration
from ...request_information import RequestInformation
from pydantic import BaseModel, Field
from typing import Union, Any, Optional
from typing import TYPE_CHECKING

if TYPE_CHECKING:
	from .get_times_off import GetTimesOffRequest
	from .get_shifts import GetShiftsRequest
	from .get_open_shifts import GetOpenShiftsRequest
	from .get_all_messages import GetAllMessagesRequest
	from .all_messages import AllMessagesRequest
	from .count import CountRequest
	from .by_team_id import ByTeamIdRequest
	from ...request_adapter import HttpxRequestAdapter
from iograph_models.beta.team import Team
from iograph_models.beta.team_collection_response import TeamCollectionResponse
from iograph_models.beta.o_data_errors__o_data_error import ODataErrorsODataError


class TeamsRequest(BaseRequestBuilder):
	def __init__(self,request_adapter: HttpxRequestAdapter, path_parameters: Optional[Union[dict[str, Any], str]]) -> None:
		super().__init__(request_adapter, "{+baseurl}/teams", path_parameters)

	async def get(
		self,
		request_configuration: Optional[RequestConfiguration[GetQueryParams]] = None,
	) -> TeamCollectionResponse:
		"""
		List teams
		List all teams in an organization.
		Find more info here: https://learn.microsoft.com/graph/api/teams-list?view=graph-rest-beta
		"""
		tags = ['teams.team']

		error_mapping: dict[str, type[BaseModel]] = {
			"XXX": ODataErrorsODataError,
		}

		request_info: RequestInformation = RequestInformation(
			method = Method.GET,
			url_template = self.url_template,
			path_parameters = self.path_parameters,
		)
		request_info.configure(request_configuration)
		request_info.headers.try_add("Accept", "application/json")
		return await self.request_adapter.send_async(request_info, TeamCollectionResponse, error_mapping)

	async def post(
		self,
		body: Team,
		request_configuration: Optional[RequestConfiguration[BaseModel]] = None,
	) -> Team:
		"""
		Create team
		Create a new team.
		Find more info here: https://learn.microsoft.com/graph/api/team-post?view=graph-rest-beta
		"""
		tags = ['teams.team']

		error_mapping: dict[str, type[BaseModel]] = {
			"XXX": ODataErrorsODataError,
		}

		request_info: RequestInformation = RequestInformation(
			method = Method.POST,
			url_template = self.url_template,
			path_parameters = self.path_parameters,
		)
		request_info.configure(request_configuration)
		request_info.headers.try_add("Accept", "application/json")
		request_info.set_content(body, "application/json")
		return await self.request_adapter.send_async(request_info, Team, error_mapping)

	class GetQueryParams(BaseModel):
		top: int = Field(default=None,serialization_alias="%24top")
		skip: int = Field(default=None,serialization_alias="%24skip")
		search: str = Field(default=None,serialization_alias="%24search")
		filter: str = Field(default=None,serialization_alias="%24filter")
		count: bool = Field(default=None,serialization_alias="%24count")
		orderby: list[str] = Field(default=None,serialization_alias="%24orderby")
		select: list[str] = Field(default=None,serialization_alias="%24select")
		expand: list[str] = Field(default=None,serialization_alias="%24expand")


	def with_url(self, raw_url: str) -> TeamsRequest:
		"""
		Returns a request builder with the provided arbitrary URL. Using this method means any other path or query parameters are ignored.
		param raw_url: The raw URL to use for the request builder.
		Returns: TeamsRequest
		"""
		if raw_url is None:
			raise TypeError("raw_url cannot be None.")
		return TeamsRequest(self.request_adapter, self.path_parameters)

	def by_team_id(self,
		team_id: str,
	) -> ByTeamIdRequest:
		if team_id is None:
			raise TypeError("team_id cannot be null.")

		path_parameters = get_path_parameters(self.path_parameters)
		path_parameters["team%2Did"] =  team_id

		from .by_team_id import ByTeamIdRequest
		return ByTeamIdRequest(self.request_adapter, path_parameters)

	@property
	def count(self,
	) -> CountRequest:
		from .count import CountRequest
		return CountRequest(self.request_adapter, self.path_parameters)

	@property
	def all_messages(self,
	) -> AllMessagesRequest:
		from .all_messages import AllMessagesRequest
		return AllMessagesRequest(self.request_adapter, self.path_parameters)

	@property
	def get_all_messages(self,
	) -> GetAllMessagesRequest:
		from .get_all_messages import GetAllMessagesRequest
		return GetAllMessagesRequest(self.request_adapter, self.path_parameters)

	@property
	def get_open_shifts(self,
	) -> GetOpenShiftsRequest:
		from .get_open_shifts import GetOpenShiftsRequest
		return GetOpenShiftsRequest(self.request_adapter, self.path_parameters)

	@property
	def get_shifts(self,
	) -> GetShiftsRequest:
		from .get_shifts import GetShiftsRequest
		return GetShiftsRequest(self.request_adapter, self.path_parameters)

	@property
	def get_times_off(self,
	) -> GetTimesOffRequest:
		from .get_times_off import GetTimesOffRequest
		return GetTimesOffRequest(self.request_adapter, self.path_parameters)

