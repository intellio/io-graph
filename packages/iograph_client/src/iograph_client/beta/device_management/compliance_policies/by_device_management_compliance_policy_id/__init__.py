# Auto-generated client

from __future__ import annotations
from kiota_abstractions.get_path_parameters import get_path_parameters
from kiota_abstractions.method import Method
from kiota_abstractions.base_request_builder import BaseRequestBuilder
from kiota_abstractions.base_request_configuration import RequestConfiguration
from .....request_information import RequestInformation
from pydantic import BaseModel, Field
from typing import Union, Any, Optional
from typing import TYPE_CHECKING

if TYPE_CHECKING:
	from .settings import SettingsRequest
	from .scheduled_actions_for_rule import ScheduledActionsForRuleRequest
	from .set_scheduled_actions import SetScheduledActionsRequest
	from .assign import AssignRequest
	from .assignments import AssignmentsRequest
	from .....request_adapter import HttpxRequestAdapter
from iograph_models.beta.o_data_errors__o_data_error import ODataErrorsODataError
from iograph_models.beta.device_management_compliance_policy import DeviceManagementCompliancePolicy


class ByDeviceManagementCompliancePolicyIdRequest(BaseRequestBuilder):
	def __init__(self,request_adapter: HttpxRequestAdapter, path_parameters: Optional[Union[dict[str, Any], str]]) -> None:
		super().__init__(request_adapter, "{+baseurl}/deviceManagement/compliancePolicies/{deviceManagementCompliancePolicy%2Did}", path_parameters)

	async def get(
		self,
		request_configuration: Optional[RequestConfiguration[GetQueryParams]] = None,
	) -> DeviceManagementCompliancePolicy:
		"""
		Get compliancePolicies from deviceManagement
		List of all compliance policies
		"""
		tags = ['deviceManagement.deviceManagementCompliancePolicy']

		error_mapping: dict[str, type[BaseModel]] = {
			"XXX": ODataErrorsODataError,
		}

		request_info: RequestInformation = RequestInformation(
			method = Method.GET,
			url_template = self.url_template,
			path_parameters = self.path_parameters,
		)
		request_info.configure(request_configuration)
		request_info.headers.try_add("Accept", "application/json")
		return await self.request_adapter.send_async(request_info, DeviceManagementCompliancePolicy, error_mapping)

	async def patch(
		self,
		body: DeviceManagementCompliancePolicy,
		request_configuration: Optional[RequestConfiguration[BaseModel]] = None,
	) -> DeviceManagementCompliancePolicy:
		"""
		Update the navigation property compliancePolicies in deviceManagement
		
		"""
		tags = ['deviceManagement.deviceManagementCompliancePolicy']

		error_mapping: dict[str, type[BaseModel]] = {
			"XXX": ODataErrorsODataError,
		}

		request_info: RequestInformation = RequestInformation(
			method = Method.PATCH,
			url_template = self.url_template,
			path_parameters = self.path_parameters,
		)
		request_info.configure(request_configuration)
		request_info.headers.try_add("Accept", "application/json")
		request_info.set_content(body, "application/json")
		return await self.request_adapter.send_async(request_info, DeviceManagementCompliancePolicy, error_mapping)

	async def delete(
		self,
		request_configuration: Optional[RequestConfiguration[BaseModel]] = None,
	) -> None:
		"""
		Delete navigation property compliancePolicies for deviceManagement
		
		"""
		tags = ['deviceManagement.deviceManagementCompliancePolicy']
		header_parameters = [{'name': 'If-Match', 'in': 'header', 'description': 'ETag', 'schema': {'type': 'string'}}]

		error_mapping: dict[str, type[BaseModel]] = {
			"XXX": ODataErrorsODataError,
		}

		request_info: RequestInformation = RequestInformation(
			method = Method.DELETE,
			url_template = self.url_template,
			path_parameters = self.path_parameters,
		)
		request_info.configure(request_configuration)
		request_info.headers.try_add("Accept", "application/json")
		return await self.request_adapter.send_no_response_content_async(request_info, error_mapping)

	class GetQueryParams(BaseModel):
		select: list[str] = Field(default=None,serialization_alias="%24select")
		expand: list[str] = Field(default=None,serialization_alias="%24expand")



	def with_url(self, raw_url: str) -> ByDeviceManagementCompliancePolicyIdRequest:
		"""
		Returns a request builder with the provided arbitrary URL. Using this method means any other path or query parameters are ignored.
		param raw_url: The raw URL to use for the request builder.
		Returns: ByDeviceManagementCompliancePolicyIdRequest
		"""
		if raw_url is None:
			raise TypeError("raw_url cannot be None.")
		return ByDeviceManagementCompliancePolicyIdRequest(self.request_adapter, self.path_parameters)

	def assignments(self,
		deviceManagementCompliancePolicy_id: str,
	) -> AssignmentsRequest:
		if deviceManagementCompliancePolicy_id is None:
			raise TypeError("deviceManagementCompliancePolicy_id cannot be null.")

		path_parameters = get_path_parameters(self.path_parameters)
		path_parameters["deviceManagementCompliancePolicy%2Did"] =  deviceManagementCompliancePolicy_id

		from .assignments import AssignmentsRequest
		return AssignmentsRequest(self.request_adapter, path_parameters)

	def assign(self,
		deviceManagementCompliancePolicy_id: str,
	) -> AssignRequest:
		if deviceManagementCompliancePolicy_id is None:
			raise TypeError("deviceManagementCompliancePolicy_id cannot be null.")

		path_parameters = get_path_parameters(self.path_parameters)
		path_parameters["deviceManagementCompliancePolicy%2Did"] =  deviceManagementCompliancePolicy_id

		from .assign import AssignRequest
		return AssignRequest(self.request_adapter, path_parameters)

	def set_scheduled_actions(self,
		deviceManagementCompliancePolicy_id: str,
	) -> SetScheduledActionsRequest:
		if deviceManagementCompliancePolicy_id is None:
			raise TypeError("deviceManagementCompliancePolicy_id cannot be null.")

		path_parameters = get_path_parameters(self.path_parameters)
		path_parameters["deviceManagementCompliancePolicy%2Did"] =  deviceManagementCompliancePolicy_id

		from .set_scheduled_actions import SetScheduledActionsRequest
		return SetScheduledActionsRequest(self.request_adapter, path_parameters)

	def scheduled_actions_for_rule(self,
		deviceManagementCompliancePolicy_id: str,
	) -> ScheduledActionsForRuleRequest:
		if deviceManagementCompliancePolicy_id is None:
			raise TypeError("deviceManagementCompliancePolicy_id cannot be null.")

		path_parameters = get_path_parameters(self.path_parameters)
		path_parameters["deviceManagementCompliancePolicy%2Did"] =  deviceManagementCompliancePolicy_id

		from .scheduled_actions_for_rule import ScheduledActionsForRuleRequest
		return ScheduledActionsForRuleRequest(self.request_adapter, path_parameters)

	def settings(self,
		deviceManagementCompliancePolicy_id: str,
	) -> SettingsRequest:
		if deviceManagementCompliancePolicy_id is None:
			raise TypeError("deviceManagementCompliancePolicy_id cannot be null.")

		path_parameters = get_path_parameters(self.path_parameters)
		path_parameters["deviceManagementCompliancePolicy%2Did"] =  deviceManagementCompliancePolicy_id

		from .settings import SettingsRequest
		return SettingsRequest(self.request_adapter, path_parameters)

