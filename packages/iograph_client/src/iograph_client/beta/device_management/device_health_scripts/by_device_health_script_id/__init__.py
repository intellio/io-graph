# Auto-generated client

from __future__ import annotations
from kiota_abstractions.get_path_parameters import get_path_parameters
from kiota_abstractions.method import Method
from kiota_abstractions.base_request_builder import BaseRequestBuilder
from kiota_abstractions.base_request_configuration import RequestConfiguration
from .....request_information import RequestInformation
from pydantic import BaseModel, Field
from typing import Union, Any, Optional
from typing import TYPE_CHECKING

if TYPE_CHECKING:
	from .run_summary import RunSummaryRequest
	from .update_global_script import UpdateGlobalScriptRequest
	from .get_remediation_history import GetRemediationHistoryRequest
	from .get_global_script_highest_available_version import GetGlobalScriptHighestAvailableVersionRequest
	from .assign import AssignRequest
	from .device_run_states import DeviceRunStatesRequest
	from .assignments import AssignmentsRequest
	from .....request_adapter import HttpxRequestAdapter
from iograph_models.beta.device_health_script import DeviceHealthScript
from iograph_models.beta.o_data_errors__o_data_error import ODataErrorsODataError


class ByDeviceHealthScriptIdRequest(BaseRequestBuilder):
	def __init__(self,request_adapter: HttpxRequestAdapter, path_parameters: Optional[Union[dict[str, Any], str]]) -> None:
		super().__init__(request_adapter, "{+baseurl}/deviceManagement/deviceHealthScripts/{deviceHealthScript%2Did}", path_parameters)

	async def get(
		self,
		request_configuration: Optional[RequestConfiguration[GetQueryParams]] = None,
	) -> DeviceHealthScript:
		"""
		Get deviceHealthScripts from deviceManagement
		The list of device health scripts associated with the tenant.
		"""
		tags = ['deviceManagement.deviceHealthScript']

		error_mapping: dict[str, type[BaseModel]] = {
			"XXX": ODataErrorsODataError,
		}

		request_info: RequestInformation = RequestInformation(
			method = Method.GET,
			url_template = self.url_template,
			path_parameters = self.path_parameters,
		)
		request_info.configure(request_configuration)
		request_info.headers.try_add("Accept", "application/json")
		return await self.request_adapter.send_async(request_info, DeviceHealthScript, error_mapping)

	async def patch(
		self,
		body: DeviceHealthScript,
		request_configuration: Optional[RequestConfiguration[BaseModel]] = None,
	) -> DeviceHealthScript:
		"""
		Update the navigation property deviceHealthScripts in deviceManagement
		
		"""
		tags = ['deviceManagement.deviceHealthScript']

		error_mapping: dict[str, type[BaseModel]] = {
			"XXX": ODataErrorsODataError,
		}

		request_info: RequestInformation = RequestInformation(
			method = Method.PATCH,
			url_template = self.url_template,
			path_parameters = self.path_parameters,
		)
		request_info.configure(request_configuration)
		request_info.headers.try_add("Accept", "application/json")
		request_info.set_content(body, "application/json")
		return await self.request_adapter.send_async(request_info, DeviceHealthScript, error_mapping)

	async def delete(
		self,
		request_configuration: Optional[RequestConfiguration[BaseModel]] = None,
	) -> None:
		"""
		Delete navigation property deviceHealthScripts for deviceManagement
		
		"""
		tags = ['deviceManagement.deviceHealthScript']
		header_parameters = [{'name': 'If-Match', 'in': 'header', 'description': 'ETag', 'schema': {'type': 'string'}}]

		error_mapping: dict[str, type[BaseModel]] = {
			"XXX": ODataErrorsODataError,
		}

		request_info: RequestInformation = RequestInformation(
			method = Method.DELETE,
			url_template = self.url_template,
			path_parameters = self.path_parameters,
		)
		request_info.configure(request_configuration)
		request_info.headers.try_add("Accept", "application/json")
		return await self.request_adapter.send_no_response_content_async(request_info, error_mapping)

	class GetQueryParams(BaseModel):
		select: list[str] = Field(default=None,serialization_alias="%24select")
		expand: list[str] = Field(default=None,serialization_alias="%24expand")



	def with_url(self, raw_url: str) -> ByDeviceHealthScriptIdRequest:
		"""
		Returns a request builder with the provided arbitrary URL. Using this method means any other path or query parameters are ignored.
		param raw_url: The raw URL to use for the request builder.
		Returns: ByDeviceHealthScriptIdRequest
		"""
		if raw_url is None:
			raise TypeError("raw_url cannot be None.")
		return ByDeviceHealthScriptIdRequest(self.request_adapter, self.path_parameters)

	def assignments(self,
		deviceHealthScript_id: str,
	) -> AssignmentsRequest:
		if deviceHealthScript_id is None:
			raise TypeError("deviceHealthScript_id cannot be null.")

		path_parameters = get_path_parameters(self.path_parameters)
		path_parameters["deviceHealthScript%2Did"] =  deviceHealthScript_id

		from .assignments import AssignmentsRequest
		return AssignmentsRequest(self.request_adapter, path_parameters)

	def device_run_states(self,
		deviceHealthScript_id: str,
	) -> DeviceRunStatesRequest:
		if deviceHealthScript_id is None:
			raise TypeError("deviceHealthScript_id cannot be null.")

		path_parameters = get_path_parameters(self.path_parameters)
		path_parameters["deviceHealthScript%2Did"] =  deviceHealthScript_id

		from .device_run_states import DeviceRunStatesRequest
		return DeviceRunStatesRequest(self.request_adapter, path_parameters)

	def assign(self,
		deviceHealthScript_id: str,
	) -> AssignRequest:
		if deviceHealthScript_id is None:
			raise TypeError("deviceHealthScript_id cannot be null.")

		path_parameters = get_path_parameters(self.path_parameters)
		path_parameters["deviceHealthScript%2Did"] =  deviceHealthScript_id

		from .assign import AssignRequest
		return AssignRequest(self.request_adapter, path_parameters)

	def get_global_script_highest_available_version(self,
		deviceHealthScript_id: str,
	) -> GetGlobalScriptHighestAvailableVersionRequest:
		if deviceHealthScript_id is None:
			raise TypeError("deviceHealthScript_id cannot be null.")

		path_parameters = get_path_parameters(self.path_parameters)
		path_parameters["deviceHealthScript%2Did"] =  deviceHealthScript_id

		from .get_global_script_highest_available_version import GetGlobalScriptHighestAvailableVersionRequest
		return GetGlobalScriptHighestAvailableVersionRequest(self.request_adapter, path_parameters)

	def get_remediation_history(self,
		deviceHealthScript_id: str,
	) -> GetRemediationHistoryRequest:
		if deviceHealthScript_id is None:
			raise TypeError("deviceHealthScript_id cannot be null.")

		path_parameters = get_path_parameters(self.path_parameters)
		path_parameters["deviceHealthScript%2Did"] =  deviceHealthScript_id

		from .get_remediation_history import GetRemediationHistoryRequest
		return GetRemediationHistoryRequest(self.request_adapter, path_parameters)

	def update_global_script(self,
		deviceHealthScript_id: str,
	) -> UpdateGlobalScriptRequest:
		if deviceHealthScript_id is None:
			raise TypeError("deviceHealthScript_id cannot be null.")

		path_parameters = get_path_parameters(self.path_parameters)
		path_parameters["deviceHealthScript%2Did"] =  deviceHealthScript_id

		from .update_global_script import UpdateGlobalScriptRequest
		return UpdateGlobalScriptRequest(self.request_adapter, path_parameters)

	def run_summary(self,
		deviceHealthScript_id: str,
	) -> RunSummaryRequest:
		if deviceHealthScript_id is None:
			raise TypeError("deviceHealthScript_id cannot be null.")

		path_parameters = get_path_parameters(self.path_parameters)
		path_parameters["deviceHealthScript%2Did"] =  deviceHealthScript_id

		from .run_summary import RunSummaryRequest
		return RunSummaryRequest(self.request_adapter, path_parameters)

