...
class DeviceActionResult(BaseModel):
	...
	def convert_discriminator_class(cls, data: Any, handler: ModelWrapValidatorHandler[Self]) -> Self:
		try:
			...
			if mapping_key == "#microsoft.graph.activateDeviceEsimActionResult":
				from .activate_device_esim_action_result import ActivateDeviceEsimActionResult
				return ActivateDeviceEsimActionResult.model_validate(data)
			if mapping_key == "#microsoft.graph.changeAssignmentsActionResult":
				from .change_assignments_action_result import ChangeAssignmentsActionResult
				return ChangeAssignmentsActionResult.model_validate(data)
			if mapping_key == "#microsoft.graph.configurationManagerActionResult":
				from .configuration_manager_action_result import ConfigurationManagerActionResult
				return ConfigurationManagerActionResult.model_validate(data)
			...
			if mapping_key == "#microsoft.graph.revokeAppleVppLicensesActionResult":
				from .revoke_apple_vpp_licenses_action_result import RevokeAppleVppLicensesActionResult
				return RevokeAppleVppLicensesActionResult.model_validate(data)
