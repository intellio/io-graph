...
class MobileAppIdentifier(BaseModel):
	...
	def convert_discriminator_class(cls, data: Any, handler: ModelWrapValidatorHandler[Self]) -> Self:
		try:
			...
			if mapping_key == "#microsoft.graph.macAppIdentifier":
				from .mac_app_identifier import MacAppIdentifier
				return MacAppIdentifier.model_validate(data)
			if mapping_key == "#microsoft.graph.windowsAppIdentifier":
				from .windows_app_identifier import WindowsAppIdentifier
				return WindowsAppIdentifier.model_validate(data)
