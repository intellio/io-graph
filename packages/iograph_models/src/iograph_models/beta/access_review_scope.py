...
class AccessReviewScope(BaseModel):
	...
	def convert_discriminator_class(cls, data: Any, handler: ModelWrapValidatorHandler[Self]) -> Self:
		try:
			...
			if mapping_key == "#microsoft.graph.accessReviewReviewerScope":
				from .access_review_reviewer_scope import AccessReviewReviewerScope
				return AccessReviewReviewerScope.model_validate(data)
			if mapping_key == "#microsoft.graph.principalResourceMembershipsScope":
				from .principal_resource_memberships_scope import PrincipalResourceMembershipsScope
				return PrincipalResourceMembershipsScope.model_validate(data)
