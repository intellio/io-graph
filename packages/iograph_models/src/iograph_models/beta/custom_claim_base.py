...
class CustomClaimBase(BaseModel):
	...
	def convert_discriminator_class(cls, data: Any, handler: ModelWrapValidatorHandler[Self]) -> Self:
		try:
			...
			if mapping_key == "#microsoft.graph.customClaim":
				from .custom_claim import CustomClaim
				return CustomClaim.model_validate(data)
			if mapping_key == "#microsoft.graph.samlNameIdClaim":
				from .saml_name_id_claim import SamlNameIdClaim
				return SamlNameIdClaim.model_validate(data)
