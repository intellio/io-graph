...
class Identity(BaseModel):
	...
	def convert_discriminator_class(cls, data: Any, handler: ModelWrapValidatorHandler[Self]) -> Self:
		try:
			...
			if mapping_key == "#microsoft.graph.programResource":
				from .program_resource import ProgramResource
				return ProgramResource.model_validate(data)
			...
			if mapping_key == "#microsoft.graph.sourceProvisionedIdentity":
				from .source_provisioned_identity import SourceProvisionedIdentity
				return SourceProvisionedIdentity.model_validate(data)
			if mapping_key == "#microsoft.graph.targetProvisionedIdentity":
				from .target_provisioned_identity import TargetProvisionedIdentity
				return TargetProvisionedIdentity.model_validate(data)
			...
			if mapping_key == "#microsoft.graph.security.submissionUserIdentity":
				from .security_submission_user_identity import SecuritySubmissionUserIdentity
				return SecuritySubmissionUserIdentity.model_validate(data)
