...
class PlannerTaskCreation(BaseModel):
	...
	def convert_discriminator_class(cls, data: Any, handler: ModelWrapValidatorHandler[Self]) -> Self:
		try:
			...
			if mapping_key == "#microsoft.graph.plannerExternalTaskSource":
				from .planner_external_task_source import PlannerExternalTaskSource
				return PlannerExternalTaskSource.model_validate(data)
			if mapping_key == "#microsoft.graph.plannerTeamsPublicationInfo":
				from .planner_teams_publication_info import PlannerTeamsPublicationInfo
				return PlannerTeamsPublicationInfo.model_validate(data)
